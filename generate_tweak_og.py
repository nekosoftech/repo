...
def create_ipa_og_image():
    ...
    # Left Preview Card (AoV Loader logo)
    aov_logo_path = "ipa/aov-loader/logo.jpg"
    card_x, card_y = 90, 115
    if os.path.exists(aov_logo_path):
        logo_im = Image.open(aov_logo_path).convert("RGBA").resize((400, 400), Image.Resampling.LANCZOS)
        logo_rounded = make_rounded(logo_im, 32)
        
        border_overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        b_draw = ImageDraw.Draw(border_overlay)
        b_draw.rounded_rectangle([card_x - 3, card_y - 3, card_x + 403, card_y + 403], radius=35, outline=(255, 255, 255, 60), width=2)
        base = Image.alpha_composite(base, border_overlay)
        base.paste(logo_rounded, (card_x, card_y), logo_rounded)
    ...
    fav_rounded = make_rounded(fav_im, 8)
    base.paste(fav_rounded, (text_x, text_y), fav_rounded)
    draw.text((text_x + 48, text_y + 6), "NekosofTech Library", fill=(0, 122, 255, 255), font=font_brand)

    draw.text((text_x, text_y + 65), "IPA Library", fill=(255, 255, 255, 255), font=font_title)

    desc_text = "Download premium modded IPA applications for iOS. Easy installation via TrollStore, AltStore, & sideloading."
    ...
    draw.text((text_x, H - 70), "https://nekosoftech.github.io/repo/ipa/", fill=(100, 116, 139, 255), font=font_meta)

    final_image = base.convert("RGB")
    os.makedirs("ipa", exist_ok=True)
    out_path = "ipa/og.png"
    final_image.save(out_path, "PNG")
    print(f"Generated solid IPA OG preview: {out_path}")
